...
class GraphManager {
  ...
  List<DataSet> makeGraphDataSets({required SavedFormData savedFormData}) {
    List<DataSet> tempList = [];
    savedFormData.savedDataSets.forEach((element) => {
          tempList.add(new DataSet(element.name ?? RandomString.randWord(),
                  colour: element.colour ?? DataSet.defaultColour)
              .setData(element.data ?? [0, 0, 0]))
        });
